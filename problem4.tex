...
\end{center}
\subsection{}
An integer AMPL model has been created for problem 4 and is included as listing \ref{lst:problem4integer}.
The data file used for this model is included as listing \ref{lst:problem4dat}. It should be noted that 
the data file is used consistently across all AMPL implementations for problem 4. The output of the integer
model can be found in listing \ref{lst:problem4integerout}
\begin{tcolorbox}
...
\subsection{}
The linear relaxation of the model is included as listing \ref{lst:problem4relaxation}. The same data file for the
integer model was used. The output is included as listing \ref{lst:problem4relaxationout}.
\begin{tcolorbox}
		      \lstinputlisting[numbers=left, numberstyle=\tiny,
			      stepnumber=2, numbersep=5pt,
...
\end{tcolorbox}
\subsection{}
The binary search tree for the Branch and Bound search is included as figure \ref{fig:problem4}. Node 27 was
identified as the optimal solution via the Branch and Bound search. The output for node 27's model is included as
listing \ref{lst:problem4node27}. The optimal production schedule for the Branch and Bound method match the optimal
solution identified by the integer ampl model in listing \ref{lst:problem4integerout}.
\begin{tcolorbox}
  \lstinputlisting[numbers=left, numberstyle=\tiny,
    stepnumber=2, numbersep=5pt,
    basicstyle=\small\fontencoding{T1}\selectfont, label={lst:problem4node27},
    caption={The AMPL output for the optimal node.}]{ampl/branchbound/node27.amplout}
\end{tcolorbox}
\begin{figure}[H]
  \centering
