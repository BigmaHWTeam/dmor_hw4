...
\end{center}
\subsection{}
An integer AMPL model has been created for problem 4 and is included as listing \eref{lst:problem4integer}.
...
\end{tcolorbox}
\subsection{}
The binary search tree for the Branch and Bound search is included as figure \ref{fig:problem4}. Node 32 was
identified as the optimal solution via the Branch and Bound search. The output for node 32's model is included as
listing \ref{lst:problem4node32}. The optimal production schedule for the Branch and Bound method match the optimal
solution identified by the integer AMPL model in listing \ref{lst:problem4integerout}. The model file and output for
each node has been included in \ref{append:p4nodes} \nameref{append:p4nodes}.
\begin{tcolorbox}
  \lstinputlisting[numbers=left, numberstyle=\tiny,
    stepnumber=2, numbersep=5pt,
